...
export default class FieldVect {
  constructor() {
    ...
    this.labelGeometry = new THREE.BufferGeometry().setAttribute(
      "position",
      new THREE.BufferAttribute(new Float32Array([0, 0.8, 0]), 3),
    );
    this.labelMaterial = new THREE.PointsMaterial({
      size: 0.08,
      sizeAttenuation: true,
      color: this.color,
      alphaMap: this.resources.items.fieldLabelTexture,
      transparent: true,
    });
  ...
  update() {
    // position
    this.vect1.position.copy(
      this.world.rightHands.hand1.getWorldPosition(new THREE.Vector3(0, 0, 0)),
    );
    this.vect2.position.copy(
      this.world.rightHands.hand2.getWorldPosition(new THREE.Vector3(0, 0, 0)),
    );
